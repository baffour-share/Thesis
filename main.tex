...
\englishkeyword{Facial Expression recognition, Depthwise Separable Convolution, Deep learning, Convolutional Neural network, Real-time}
\end{englishabstract}
...
\begin{itemize}
    ...
    \item Chapter 2 explains all theoretical basics underlining the proposed idea in the thesis.
    \item Chapter 3 discusses the proposed method and the various experiments carried out in the laboratory.
    \item Chapter 4 discusses the results of the experiments carried out. Also, the chapter presents the results metrics used to evaluate the experiments. Finally, this chapter analyses in details the results obtained on experiments and real-time testing.
    \item Chapter 5 provides the concluding remarks and some future works.
\end{itemize}
...
\caption{Directed graph showing ANN with input units \textbf{A, B,} and \textbf{C} into the neuron \textbf{S} to perform computation on weights \textbf{W ($w_1, w_2, w_3$)} with output \textbf{$Y$}: Source:\citing{McGonagle}}
\label{fig_neuron}
\end{figure}
...
\caption{A simple ANN consisting of an input layer (Layer 0) with inputs $x_1, x_2, x_3$ and weights $w_{x_1s_1}, w_{x_2s_1}, w_{x_2s_2}, w_{x_3s_2}$; and an output layer (Layer 3) with inputs $s_3, s_4, s_5$ and weights $w_{s_3y_1}, w_{s_4y_1}, w_{s_5y_2}$ and two hidden layers (Layer 1 and Layer 2) in between. Source:\citing{McGonagle}}
\label{fig_ANN}
\end{figure}
...
\caption{An illustration of how gradient descent algorithm minimizes the function to the downhill. Source: \citing{Goodfellow-et-al-2016}}
...
\caption{ConvNet recognizing scenes and accurately suggesting important captions: Source \citing{densecap}}
\label{cnn_example}
\end{figure}

\begin{figure}[ht]
\includegraphics[width=5in]{pic/everyday_objects.PNG}
\caption{ConvNet recognizing everyday objects with bounding boxes: Source \citing{DBLP:journals/corr/RenHG015}}
\label{everyday_objects}
\end{figure}

\begin{figure}[ht]
\includegraphics[width=5in]{pic/ConvNet_Arch.png}
\caption{A simple ConvNet architecture: Source \citing{intuitiveCNN}}
\label{ConvNet_arch}
\end{figure}
...
\caption{An RGB image with volume of size: 4 x 4 x 3. It comprises of the 3 Colour channel matrices: Source \citing{Abhineet}}
\label{rgb_image}
\end{figure}
...
Our proposed model structure is a pure convolutional network (i.e. without any fully-connected layer) that is based on depthwise separable convolutions as explained in section \ref{depthconve}. All depthwise separable layers perform \textit{same convolution} with a filter size of $3 \times 3 \times 1$ and a \textit{depth-multiplier} value of $1$ shifting by $1$ stride. We use ReLU\citing{7410480} activation function in all the separable convolutional layers to help us avoid the vanishing gradient problem\citing{5264952}. All depthwise separable convolutional layers are followed by a batch normalization \citing{DBLP:journals/corr/IoffeS15} operation to normalize the output in each layer in order to avoid varying distributions of the features across the training and test data. For our model to generalize well on unseen data and avoid overfitting on the training data, we apply dropout\citing{JMLR:v15:srivastava14a} as our regularization method in the last three depthwise separable convolutional layers. We apply Max-Pooling of window size $2 \times 2$ and stride $1$ after the $6^{th}$, $9^{th}$, $12^{th}$, and $15^{th}$ depthwise separable convolutional layers in order to down sample input flowing through the network. Instead of the traditional fully connected layer, we output the spatial average of the learned feature maps from the last depthwise separable convolutional layer via a global average pooling layer \citing{DBLP:journals/corr/LinCY13}; and the output feature vector is fed into the softmax layer for classification.  In summary, the entire network contains only $474K$ parameters with 15 separable convolutional layers and four pooling layers. Table \ref{model_arch} shows details of our proposed model architecture.
...
\caption{Examples of facial expression images from the Japanese Female Facial Expression (JAFFE) database. Source: \citing{Michael-2017}}
\label{jaffe_images}
\end{figure}
...
are: (a) Disgust - AU 1+4+15+17, (b) Happy - AU 6+12+25, (c) Surprise - AU 1+2+5+25+27, (d) Fear - AU 1+4+7+20, (e) Angry - AU 4+5+15+17, (f) Contempt - AU 14, (g) Sadness - AU 1+2+4+15+17, and (h) Neutral - AU0. Source: \citing{5543262}}
\label{CK+_images}
\end{figure}

\subsubsection{Extended Cohn-Kanade Dataset}
The Extended Cohn-Kanade Dataset (CK+) dataset\citing{5543262} is a build on the work done by \citing{ck_840611}. In the work done by \citing{ck_840611}, they presented a sequence of 2105 images obtained from 128 adult subjects of different ethnicity. The subjects expressed the various emotions by acting the multiple tokens of many basic Facial Action Coding System (FACS)\citing{ekman_facs} action units. The Extended Cohn-Kanade Dataset (CK+) dataset increased the sequence of images by $22\%$ and the also increased the number of adult subjects by $27\%$. All label expressions for each sequence was fully based on FACS coding. Also, CK+ dataset added non-posed sequences for many kinds of smiles and their corresponding metadata. Examples of facial expression images from the CK+ dataset are shown in figure \ref{CK+_images}
...
\end{itemize}
We maintained a batch size of 32 across all datasets to feed input into the network. We trained the network using Adam Optimizer \citing{DBLP:journals/corr/KingmaB14} with learning rate of 0.001 for both datasets. Training results are presented in chapter \ref{chp4}. Finally, we implemented our model with OpenCV on an Intel Dual Core i3-7100U 2.4GHz Processor laptop in real-time by connecting a live video stream to a face detector to feed images to our trained model to recognise the 7 basic emotions.
...
The winner of the Kaggle competition \citing{tang2018} achieved 69.4\% accuracy on FERC-2013 public test and 71\% accuracy on the public test with a model containing more than 7 million parameters. Sang et al \citing{sang-2017} also achieved 71\% on FERC-2013 public test and 71.9\% on the private test with a model containing 4.19 million parameters. Arriaga et al\citing{DBLP:journals/corr/abs-1710-07557} achieved 66\% accuracy on FERC-2013 public test with a model containing approximately 600K parameters. Our proposed model contains $474K$ parameters, nevertheless, we achieved only $2\%$ lower than \citing{tang2018}, 2.9\% lower than \citing{sang-2017} and 3\% higher than \citing{DBLP:journals/corr/abs-1710-07557} on FERC-2013 private test. Our proposed model contains approximately 6.5 million fewer parameters compared to \citing{tang2018}, 4.2 million fewer parameters compared to \citing{sang-2017} and 126K fewer parameters compared to \citing{DBLP:journals/corr/abs-1710-07557}.

Duncan et al \citing{duncan2016} achieved 62\% accuracy on JAFFE dataset with a highly parameterized CNN model containing more than 10 million parameters. Additionally, \citing{7988558} achieved 87\% accuracy on JAFFE dataset with a model containing more than 10 million parameters. However, we achieved $88\%$ accuracy on JAFFE dataset with our proposed model containing about a whooping 9 million fewer parameters compared to \citing{duncan2016} and \citing{7988558}.

For CK+ dataset, Pooya et. al. \citing{do_deep_7406361} achieved $96.7\%$ accuracy using quadrant pooling technique on a CNN model containing a little over 4 million parameters. Ghosh et. al. \citing{Ghosh_7344632} achieved accuracy up to $88.14\%$ using a multi-label CNN model containing only about $321K$ total parameters. Table \ref{table_comparison} shows the comparison summary of our proposed model with other models. 
...
The software was ran and tested on a Windows laptop with Windows 10, python, keras, CUDA (Compute Unified Device Architecture) and CuDNN (CUDA Deep Neural Netowrk) installed. The laptop has a web cam support. The software dependicies includes:
\begin{enumerate}
...
\section{Conclusion}
In this chapter, we present the evaluation metrics used to measure the proposed model performance. This chapter also provides analysis of the model performance on all three datasets. Comparison of the proposed method with other state-of-the-art methods is also presented in this chapter. Finally, this chapter presents the real-time application of the proposed method. 
...
% from keras.layers import Activation, Dropout, Dense, GlobalAveragePooling2D, Conv2D, Flatten,
%     ReLU, DepthwiseConv2D, MaxPool2D, AveragePooling2D
% from keras.layers import MaxPooling2D, SeparableConv2D, BatchNormalization
% from keras.models import Sequential
% from keras.preprocessing.image import ImageDataGenerator
% from sklearn.metrics import classification_report, confusion_matrix

% # seed ramdomnly generated numbers
% np.random.seed(42)
% rn.seed(12345)
...
% train_data_dir = './fer2013/Training'
% validation_data_dir = './fer2013/PublicTest'
% private_test_data_dir = './fer2013/PrivateTest'
...
% class_label_names = ['Anger', 'Disgust', 'Fear', 'Happy', 'Neutral', 'Sad', 'Surprise']
...
%     #  1st layer
%     my_model.add(SeparableConv2D(filters=32, kernel_size=(kernel_height, kernel_width),
%                                  padding='same', strides=(1, 1), depth_multiplier=
%                                  depth_multiplier, use_bias=True, data_format=
%                                  'channels_last',input_shape=image_shape,activation='relu'))
...
%                                  depth_multiplier,use_bias=True, data_format=
%                                  'channels_last', activation='relu'))
%     my_model.add(AveragePooling2D(2, 2))
%     my_model.add(BatchNormalization())
...
%     my_model.add(SeparableConv2D(filters=32, kernel_size=(kernel_height, kernel_width),
%                                  padding='same', depth_multiplier=depth_multiplier,
%                                  use_bias=True,data_format='channels_last',activation='relu'))
%     my_model.add(AveragePooling2D(2, 2))
%     my_model.add(BatchNormalization())
...
%     my_model.add(SeparableConv2D(filters=32, kernel_size=(kernel_height, kernel_width),
%                                  padding='same', strides=(1, 1), depth_multiplier=
%                                  depth_multiplier,use_bias=True, data_format='channels_last',
%                                  activation='relu'))
%     my_model.add(BatchNormalization())
...
%     my_model.add(BatchNormalization())
%     my_model.add(Dropout(0.10))

%     # softmax layer
%     my_model.add(Dense(num_labels, activation='softmax'))
...
%     server.starttls()
%     server.login("baffour205@outlook.com", "**************")

%     msg = "Training finished!"
%     server.sendmail("baffour205@outlook.com", "asarebaffour700@gmail.com", msg)
%     server.quit()

% model = trial_model_best()
% model.summary()
...
%     color_mode='grayscale',
%     class_mode='categorical')
% number_of_test_samples = len(validation_generator.filenames)

% private_test_generator = test_datagen.flow_from_directory(
%     private_test_data_dir,
%     target_size=(img_width, img_height),
%     batch_size=batch_size,
%     color_mode='grayscale',
%     class_mode='categorical')
% number_of_private_test_samples = len(private_test_generator.filenames)

% # record model history
% tensor_board = TensorBoard(log_dir='./logs', histogram_freq=0,
%                             write_graph=True, write_images=True)
% # save model
% check_pointer = ModelCheckpoint(MODEL_PATH, monitor='val_loss', verbose=1,
%                                 save_best_only=True)

% history = model.fit_generator(train_generator, epochs=epochs, validation_data=
%                             validation_generator,callbacks=[tensor_board,
%                             check_pointer], validation_steps=112, verbose=1,
%                             steps_per_epoch=897)
% send_report()
...
% with open("./models/fer2013_model_.json", "w") as json_file:
%     json_file.write(model_json)

% # evaluate model
% scores = model.evaluate_generator(validation_generator, steps=112,
%                                 workers=1, verbose=1)
...
% number_of_generator_calls = np.math.ceil(
%                             number_of_test_samples / (1.0 * batch_size))
% for i in range(0, int(number_of_generator_calls)):
%     test_labels.extend(np.array(validation_generator[i][1]))

% number_of_private_test_generator_calls = np.math.ceil(
%                             number_of_private_test_samples / (1.0 * batch_size))
% for i in range(0, int(number_of_private_test_generator_calls)):
%     private_test_labels.extend(np.array(private_test_generator[i][1]))

% predictions = model.predict_generator(validation_generator,steps=112,verbose=0)
% private_test_predictions = model.predict_generator(
%                                 private_test_generator, steps=112, verbose=0)

% y_pred = np.argmax(predictions, axis=1)
% y_pred_private = np.argmax(private_test_predictions, axis=1)

% # print recall, precision, and f1-score
% print(classification_report(np.argmax(test_labels, axis=1), 
%                             y_pred,target_names=class_label_names))
% print(classification_report(np.argmax(private_test_labels, axis=1), 
%                             y_pred_private, target_names=class_label_names))

% # print confusion matrix
% print(confusion_matrix(np.argmax(test_labels, axis=1), y_pred))
% print(confusion_matrix(np.argmax(private_test_labels,axis=1),y_pred_private))
...
% plot_confusion_matrix(cnf_matrix, classes=class_label_names,
%                         title='Public test Confusion matrix')
% plt.figure()
% plt.show()

% # Plot non-normalized confusion matrix
% plot_confusion_matrix(cnf_matrix, classes=class_label_names, normalize=True,
%                       title='Public test normalized Confusion matrix')
% plt.figure()
% plt.show()

% # Compute confusion matrix
% cnf_matrix = (confusion_matrix(np.argmax(private_test_labels, axis=1), y_pred_private))
% np.set_printoptions(precision=2)
% plt.figure()

% # Plot non-normalized confusion matrix
% plot_confusion_matrix(cnf_matrix, classes=class_label_names,
%                         title='Private Test Confusion matrix')
% plt.figure()
% plt.show()

% # Plot non-normalized confusion matrix
% plot_confusion_matrix(cnf_matrix, classes=class_label_names, normalize=True,
%                       title='Private test Normalized Confusion matrix')
% plt.figure()
% plt.show()
% \end{python}
...
% MODEL_PATH = os.getcwd() + '/models/jaffe_model.h5'
% emotion_dict = {0: "Angry", 1: "Disgust", 2: "Fear", 3: "Happy", 4: "Sad",
